...
{
    public class LedgerRevenueExpenseDAL
    {
        ReportDataContext objDataContext;
        public List<EAccountStatement> GetAllJournalByAccount(DateTime fromDate, DateTime toDate, string accTitle, long accId, DateTime fiscalStartDate)
        {
            ...
            objEASBasicInfo.AccountCurrentBalance = GetCurrentBalanceOfAccount(accId, fromDate,fiscalStartDate);//, ref IsCreationDateUse);
        ...
        private string GetAddress(long accId)
        {
            string address = null;
            objDataContext = new ReportDataContext();
            var query = from j in objDataContext.CUSTOMER_EMPLOYEE_SETUPs where j.ACCOUNT_ID == accId select j;
        ...
        public decimal GetCurrentBalanceOfAccount(long accId, DateTime fromDate, DateTime fiscalStartDate)//, ref bool IsCreationDateUse)
        {
            objDataContext = new ReportDataContext();
            decimal openBalance = 0;
            decimal amount = 0;
            foreach (var c in objDataContext.SP_CURRENT_BALANCE_REVENUE_EXPENSE(accId,fiscalStartDate,fromDate))
            {
